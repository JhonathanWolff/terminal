...
def run(project: str, cliente: str, api_selecionada: str, reports: list):
    ...
    if re.match(r"\d+$", start):
        start = (datetime.datetime.now() -
                 datetime.timedelta(days=int(start))).strftime(format_date)

    if re.match(r"\d+$", end):
        end = (datetime.datetime.now() -
               datetime.timedelta(days=int(end))).strftime(format_date)
    ...
    if (datetime.datetime.strptime(start, format_date) > datetime.datetime.strptime(end, format_date)
        ...
        start, format_date).date() > datetime.datetime.now().date()
        or
        datetime.datetime.strptime(
        end, format_date).date() > datetime.datetime.now().date()
        ):
    ...
    firestore_ids = firestore_list_collection.list_collection(
...
def build_payload(api, client,
    ...
    if collection_id is None:
        ...
        obj["endpoints"][api][resource][collection_key][collection_id] = [
...
def get_multi_fzf(text):
    ...
    temp_fname = str(uuid.uuid4()).replace("-", "")
    temp_fname = f"/tmp/{temp_fname}"

    with open(temp_fname, "w") as f:
        f.write(text)

    subprocess.run(['nvim', temp_fname], check=True)
...
Current Configs
    ...
    * Reports [{reports}]
    ...
    run(project=project_id, cliente=cliente,
        api_selecionada=api_selecionada, reports=reports.split(","))
